...
function requestSearchBook(data, successCb, errorCb, completeCb) {
    ...
    //request(api.API_BOOK_SEARCH.replace('searchTitle',data), data, successCb, errorCb, completeCb);
}

/**
 * 获取图书详细信息
 */
function requestBookDokDetail(id,successCb, errorCb, completeCb) {
    request(api.API_BOOK_DETAIL.replace('pk', id),successCb, errorCb, completeCb);
}
